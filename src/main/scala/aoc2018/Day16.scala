...
class Day16 extends Day(2018, 16) {
  ...
  override def part1: String = {
    inputGroups.dropRight(3).count { g =>
  ...
  override def part2: String = {
    ...
    while (knownOps.size != 16) {
      inputGroups.dropRight(3).foreach { g =>
        var s: State = State(Array(0, 0, 0, 0))
        var i: Instruction = Instruction(0, 0, 0, 0)
        var o: State = State(Array(0, 0, 0, 0))
        g.foreach {
    ...
    inputGroups.takeRight(1).foreach { g =>
      g.foreach {
        case instruction(op: String, a: String, b: String, c: String) =>
...
object Day16 {
  ...
  val before = raw"Before: \[(\d), (\d), (\d), (\d)\]".r
  val instruction = raw"(\d+) (\d) (\d) (\d)".r
  val after = raw"After:  \[(\d), (\d), (\d), (\d)\]".r
}
