...
class Day24 extends Day(2016, 24) {
  override def part1: String = {
    ...
    locations.keys.toList.combinations(2).foreach { xs =>
      ...
      ptDistances.addOne((xs(0), xs(1)), path.length - 1)
      ptDistances.addOne((xs(1), xs(0)), path.length - 1)
    }
  ...
  def allSurroundings(x: Int, y: Int, grid: Array[Array[Char]]) = Seq((x - 1, y - 1), (x, y - 1), (x + 1, y - 1), (x - 1, y), (x + 1, y), (x - 1, y + 1), (x, y + 1), (x + 1, y + 1)).map(c => grid(c._2)(c._1)).count(_ == '#')
